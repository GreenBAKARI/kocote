...
// クエリの発行
$sql_result_ua = mysql_query ( 'SELECT * FROM ua' );
if (! $sql_result_ua) {
	die ( 'クエリ失敗。' . mysql_error () );
}

// 連想配列としてすべての行の各フィールドの値を出力
while ( $row = mysql_fetch_assoc ( $sql_result_ua ) ) {
	print ($row ['USER_ID']) ;
...
<!-- 画像アップロード -->
	...
	<FORM method="POST" action="mypage_detail.php">
		<P>画像表示</P>
		ID：<INPUT type="text" name="id"> <INPUT type="submit" name="submit"
			value="送信"> <BR> <BR>
	</FORM>
